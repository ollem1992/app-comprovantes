    ...
    if st.button("Processar Comprovantes"):
        with st.spinner("Lendo planilha e cruzando dados..."):
            ...
            try:
                ...
                # Garante que a coluna UF existe (compatibilidade com planilhas antigas)
                ...
                for _, row in df.iterrows():
                    uf = str(row.get('UF', '')).strip().upper() if tem_uf else ''
                    v1 = row['_v1']
                    v2 = row['_v2']
                    jr = row['_jr']
                    ...
                    # --- REGRA ALAGOAS (AL) ---
                    # AL emite ICMS e FECP em comprovantes separados.
                    # Cada valor é registrado individualmente no dicionário de busca,
                    # com sufixo _ICMS e _FECP para diferenciar os arquivos no ZIP.
                    if uf == 'AL':
                        if v1 > 0:
                            chave_icms = round(v1 + jr, 2)
                            valor_para_nomes[chave_icms].append(f"{nome}_ICMS")
                        if v2 > 0:
                            valor_para_nomes[round(v2, 2)].append(f"{nome}_FECP")
                    # --- REGRA PADRÃO (demais estados) ---
                    # Soma VALOR 1 + VALOR 2 + JUROS para buscar um único comprovante.
                    else:
                        chave = round(v1 + v2 + jr, 2)
                        if chave > 0:
                            valor_para_nomes[chave].append(nome)
